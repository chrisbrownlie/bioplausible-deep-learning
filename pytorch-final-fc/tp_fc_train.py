...
def train_forward_parameters(args, net, predictions, targets, loss_function,
    ...
    if args.classification:
        if args.output_activation == 'sigmoid':
            batch_accuracy = compute_accuracy(predictions,
                                            one_hot_to_int(targets))
        else: #softmax
...
def train_bp(args, device, train_loader, net, writer, test_loader, summary,
        ...
        if args.classification:
            if args.output_activation == 'sigmoid':
                ...
                raise NetworkError("output space plot for classification "
                                     "tasks is only possible with sigmoid "
                                     "output layer.")
        make_plot_output_space_bp(args, net,
                                     args.output_space_plot_layer_idx,
                                     loss_function,
                                     targets,
                                     inputs,
    ...
    for e in range(args.epochs):
        ...
        for i, (inputs, targets) in enumerate(train_loader):
            ...
            if args.classification:
                if args.output_activation == 'sigmoid':
                    running_accuracy += compute_accuracy(predictions,
                                                    one_hot_to_int(
                                                        targets))
                else:  # softmax
                    running_accuracy += compute_accuracy(predictions, targets)
