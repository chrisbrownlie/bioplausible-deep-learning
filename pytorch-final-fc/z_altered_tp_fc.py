...
def choose_optimizer(net):
    ...
    Returns: optimizer
    """
    forward_optimizer = OptimizerList(net)
    # feedback_optimizer = choose_feedback_optimizer(args, net)
    feedback_optimizer = FbOptimizerList(net)

    return forward_optimizer, feedback_optimizer


def choose_forward_optimizer(net):
    """
    Return the wished optimizer (based on inputs from args).
    Args:
        args: cli
...
def choose_feedback_optimizer(net):
    """
    Return the wished optimizer (based on inputs from args).
    Args:
        args: cli
...
class OptimizerList(object):
    ...
    def __init__(self, net):
        forward_optimizer = choose_forward_optimizer(net)
...
class FbOptimizerList(object):
    def __init__(self, net):
        fb_optimizer = choose_feedback_optimizer(net)
...
def setup_summary_dict():
    ...
    save_summary_dict(summary)

    return summary


def save_summary_dict(summary):
    ...
    if not os.path.isdir('tp_meul'):
        os.mkdir('tp_meul')

    with open(os.path.join('tp_meul', summary_fn), 'w') as f:
...
def make_plot_output_space(args, net, i, loss_function,
    ...
    net.backward(loss, 0.01, save_target=False,
                 norm_ratio=1.)
    ...
    plt.savefig(os.path.join('tp_meul', file_name))
    plt.close()

    file_name = 'output_arrow_' + args.network_type + '.npy'
    np.save(os.path.join('tp_meul', file_name),
            output_arrow)
...
def make_plot_output_space_bp(net, i, loss_function,
                           targets, inputs, steps=20):
    """
    Make a plot of how the output activations would change if the update
    for the parameters of layer(s) i is applied with a varying stepsize from
    ...
    file_name = 'output_space_updates_fig_DTP.svg'
    plt.savefig(os.path.join('tp_meul', file_name))
    plt.close()
    # file_name = 'output_space_updates_traj_' + args.network_type + '.npy'
    # np.save(os.path.join('tp_meul', file_name), output_traj)
    file_name = 'output_arrow_DTP.npy'
    np.save(os.path.join('tp_meul', file_name),
            output_arrow)
    file_name = 'output_arrow_start_DTP.npy'
    np.save(os.path.join('tp_meul', file_name),
            output_arrow_start)
    file_name = 'output_space_label_DTP.npy'
    np.save(os.path.join('tp_meul', file_name),
            targets[0, 0:2].detach().cpu().numpy())
...
class DTPLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True,
        ...
        # orthogonal initialisation per Lee 2015
...
######### TRAINING #############
################################
def train(train_loader, net, test_loader, summary):
    """
    ...
    train_var.forward_optimizer = torch.optim.SGD(net.parameters(), lr = 0.001, momentum = 0.9)
    train_var.feedback_optimizer = torch.optim.SGD(net.parameters(), lr = 0.001, momentum = 0.9)
    train_var.loss_function = torch.nn.CrossEntropyLoss()
    ...
    for e_fb in range(1):
        ...
        train_only_feedback_parameters(train_var, train_loader,
                                       net)
    ...
    print(f'Initialization feedback weights done after {1} epochs.')
    ...
    for e in range(100):
        ...
        # Train separate as in Lee 2015
        train_separate(train_var, train_loader, net)

        for extra_e in range(0):
            train_only_feedback_parameters(train_var, train_loader,
                                            net, log=False)
        ...
        save_summary_dict(train_var.summary)
...
def train_separate(train_var, train_loader, net):
    """
    ...
    for i, (inputs, targets) in enumerate(train_loader):
            ...
            train_feedback_parameters(net, train_var.feedback_optimizer)
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        train_var.batch_accuracy, train_var.batch_loss = \
            train_forward_parameters(net, predictions, targets,
                                     train_var.loss_function,
...
def train_forward_parameters(net, predictions, targets, loss_function,
                             forward_optimizer):
    ...
    net.backward_random(loss, 0.01, k,
                        save_target=save_target, norm_ratio=1.)
...
def train_feedback_parameters(net, feedback_optimizer):
...
def train_only_feedback_parameters(train_var, train_loader,
                                   net, log=True):
...
""" model = DTPNetwork(n_in=3072, n_hidden=[1536, 256], n_out = 10)
print(model.layers)
